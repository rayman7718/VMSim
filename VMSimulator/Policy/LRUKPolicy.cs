...
namespace VMSimulator
{
	...
	public LRUKPolicy(int K)
	{
        ...
        public Dictionary<int, List<VMInfoBase>> GetNewConfig(ReconfigureEvent e, Dictionary<int, List<VMInfoBase>> currentConfig, Dictionary<VMId, VMInfoBase> vminfos, ICostMatrix costmatrix)
        {
        ...
        private List<VMInfoBase> GetLeastRecentlyUsed(List<VMInfoBase> VMList, int n)
        {
            ...
            VMList.Sort(
                delegate(VMInfoBase a, VMInfoBase b)
                {
                    ...
                    return a.ArrivalTimestamp[a.ArrivalTimestamp.Count-K].CompareTo(b.ArrivalTimestamp[b.ArrivalTimestamp.Count-K]);
                }
                );
